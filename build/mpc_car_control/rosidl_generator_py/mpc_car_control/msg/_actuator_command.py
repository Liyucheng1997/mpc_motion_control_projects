    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'mpc_car_control.msg.ActuatorCommand')
            logger.debug(
...
class ActuatorCommand(metaclass=Metaclass_ActuatorCommand):
    ...
    def __init__(self, **kwargs):
        ...
        if 'wheel_torque' not in kwargs:
            ...
            self.wheel_torque = numpy.array(kwargs.get('wheel_torque'), dtype=numpy.float64)
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'wheel_torque' field must be a set or sequence with length 4 and each value of type 'float'"
